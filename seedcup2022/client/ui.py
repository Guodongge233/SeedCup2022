...
item2emoji1 = {BuffType.BuffHp: 1, BuffType.BuffSpeed: 2}
...
slave2emoji1 = {
    SlaveWeaponType.Kiwi: 1,
    SlaveWeaponType.Cactus: 2,
}
    ...
    def get_info(self):
        ...
        if self.valid:
            if self.obj == ObjType.Null:
                return [color2emoji1[self.color], 0, 0, 0]
            elif self.obj == ObjType.Character:
                if not self.data.isAlive:
                    return [0, 5, 0, 0]
                return [0, self.data.playerID, 0, 0]
            elif self.obj == ObjType.Item:
                return [0, 0, item2emoji1[self.data.buffType],0]
            elif self.obj == ObjType.SlaveWeapon:
                return [0, 0, 0, slave2emoji1[self.data.weaponType]]
        else:
            return [color2emoji1[ColorType.Black], 0, 0, 0]
...
class UI(object):
    ...
    def getmap(self, x, y):
        ...
        return self._blocks[x][y].get_info()
